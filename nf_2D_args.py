...
import argparse
parser = argparse.ArgumentParser(description='Normalizing flows 2D')
...
for i in range(args.flow_steps):
    ...
    m1 = CPABTransform2D(True, **kwargs)
    m2 = CPABTransform2D(False, **kwargs)
    transforms.append(m1)
    transforms.append(m2)
...
alpha = 12
low = torch.FloatTensor([alpha, alpha]).to(device)
high = torch.FloatTensor([alpha, alpha]).to(device)
...
@torch.no_grad()
def plot_2D(model, train_loader, target_distribution, path, n_samples=2000):
    ...
    for k in range(n):
        ...
        # axs[k].axis("equal")
        axs[k].axis("off")
    axs[0].set_title(f"$p(x)$")
    axs[1].set_title(f"$p(z)$")
    ...
    axs[0].set_title(f"$p(z)$")
    axs[1].set_title(f"$p(x)$")
...
@torch.no_grad()
def plot_evolution_2D(model, train_loader, target_distribution, path, n_samples=2000):
    # TRANSFORMATION EVOLUTION
...
plot_2D(model, train_loader, target_distribution, path, n_samples=3000)

plot_evolution_2D(model, train_loader, target_distribution, path, n_samples=3000)
